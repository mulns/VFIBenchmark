...
def backwarp(tenInput, tenFlow):
    ...
    tenInput = torch.cat([tenInput, backwarp_tenPartial[str(tenFlow.shape)]],
                         1)
...
class Network(torch.nn.Module):
            ...
            def forward(self, tenFirst, tenSecond, objPrevious):
                ...
                if objPrevious is None:
                    ...
                    tenVolume = torch.nn.functional.leaky_relu(
                        input=correlation.FunctionCorrelation(
                            tenFirst=tenFirst,
                            tenSecond=backwarp(tenInput=tenSecond,
                                               tenFlow=tenFlow *
                                               self.fltBackwarp)),
                        negative_slope=0.1,
                        inplace=False)
        ...
        class Refiner(torch.nn.Module):
            ...
            def __init__(self):
                ...
                self.netMain = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=81 + 32 + 2 + 2 + 128 + 128 +
                                    96 + 64 + 32,
                                    out_channels=128,
                                    kernel_size=3,
                                    stride=1,
    ...
    def forward(self, tenFirst, tenSecond):
        ...
        tenFlow = objEstimate['tenFlow'] + self.netRefiner(
            objEstimate['tenFeat'])
