...
class Efficient_HetConv2d(nn.Module):
    def __init__(self, in_feats, out_feats, p=4, ks=3, pad=1):
        ...
        self.conv3x3 = nn.Conv2d(in_feats,
                                 out_feats,
                                 kernel_size=ks,
                                 padding=pad,
                                 groups=p)
...
class Network(nn.Module):
    ...
    def __init__(self,
        ...
        def Basic(intInput, intOutput, ks, pad):
            return torch.nn.Sequential(
                Efficient_HetConv2d(in_feats=intInput,
                                    out_feats=intOutput,
                                    p=self.het_p,
                                    ks=ks,
                                    pad=pad), torch.nn.ReLU(inplace=False),
                Efficient_HetConv2d(in_feats=intOutput,
                                    out_feats=intOutput,
                                    p=self.het_p,
                                    ks=ks,
                                    pad=pad), torch.nn.ReLU(inplace=False),
        ...
        def Upsample(intInput, intOutput):
            return torch.nn.Sequential(
                ...
                Efficient_HetConv2d(in_feats=intOutput,
                                    ...
                                    p=self.het_p,
                                    ks=3,
                                    pad=1), torch.nn.ReLU(inplace=False))

        # end

        def KernelNet():
            return torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=self.estimator_in,
                                out_channels=64,
                                kernel_size=3,
                                stride=1,
                ...
                torch.nn.Upsample(scale_factor=2,
                                  mode='bilinear',
                                  align_corners=True),
                torch.nn.Conv2d(in_channels=self.generated_ks,
                                out_channels=self.generated_ks,
                                kernel_size=3,
                                stride=1,
                                padding=1))
        ...
        def Offsetnet():
            return torch.nn.Sequential(
                ...
                torch.nn.Upsample(scale_factor=2,
                                  mode='bilinear',
                                  align_corners=True),
                torch.nn.Conv2d(in_channels=self.generated_ks**2,
                                out_channels=self.generated_ks**2,
                                kernel_size=3,
                                stride=1,
                                padding=1))
        ...
        self.moduleConv1 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=6,
                            ...
                            stride=1,
                            padding=1), torch.nn.ReLU(inplace=False),
            Efficient_HetConv2d(in_feats=32,
                                out_feats=32,
                                p=self.het_p,
                                ks=3,
                                pad=1), torch.nn.ReLU(inplace=False),
    ...
    def forward(self, tensors):
        ...
        tensorConv2 = self.moduleConv2(
            torch.nn.functional.avg_pool2d(input=tensorConv1,
                                           kernel_size=2,
                                           stride=2))
        tensorConv3 = self.moduleConv3(
            torch.nn.functional.avg_pool2d(input=tensorConv2,
                                           kernel_size=2,
                                           stride=2))
        tensorConv4 = self.moduleConv4(
            torch.nn.functional.avg_pool2d(input=tensorConv3,
                                           kernel_size=2,
                                           stride=2))
        tensorConv5 = self.moduleConv5(
            torch.nn.functional.avg_pool2d(input=tensorConv4,
                                           kernel_size=2,
                                           stride=2))

        tensorDeconv5 = self.moduleUpsample5(
            self.moduleDeconv5(
                torch.nn.functional.avg_pool2d(input=tensorConv5,
                                               kernel_size=2,
                                               stride=2)))
        ...
        if self.isMultiple:
            v1 = self.moduleVertical1(torch.cat([tensorCombine, tensorTime],
                                                1))
            v2 = self.moduleVertical2(
                torch.cat([tensorCombine, 1. - tensorTime], 1))
            h1 = self.moduleHorizontal1(
                torch.cat([tensorCombine, tensorTime], 1))
            h2 = self.moduleHorizontal2(
                torch.cat([tensorCombine, 1. - tensorTime], 1))

            tensorDot1 = dsepconv.FunctionDSepconv(
                tensorFirst, v1, h1,
            ...
            tensorDot2 = dsepconv.FunctionDSepconv(
                tensorSecond, v2, h2,
                self.moduleOffset2x(
                    torch.cat([tensorCombine, 1. - tensorTime], 1)),
                self.moduleOffset2y(
                    torch.cat([tensorCombine, 1. - tensorTime], 1)),
                self.moduleMask2(torch.cat([tensorCombine, 1. - tensorTime],
                                           1)))
        else:
