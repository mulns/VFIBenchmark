...
class Model:
    ...
    def load_model(self, name=None, rank=0):
        ...
        if rank <= 0:
            if name is None:
                name = self.name
            self.net.load_state_dict(convert(torch.load(f"checkpoints/{name}.pkl")))
    ...
    @torch.no_grad()
    def inference_test(self, img0, img1, timestep=0.5):
        """
        Noting: return BxCxHxW
        """
        ...
        imgs = torch.cat((img0, img1), 1)
        # _, _, _, pred = self.net(imgs, timestep=timestep)
        flows, masks = self.net.calculate_flow(imgs, timestep=timestep)
