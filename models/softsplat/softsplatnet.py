...
def read_flo(strFile):
    ...
    assert (numpy.frombuffer(buffer=strFlow,
                             dtype=numpy.float32,
                             count=1,
                             offset=0) == 202021.25)

    intWidth = numpy.frombuffer(buffer=strFlow,
                                dtype=numpy.int32,
                                count=1,
                                offset=4)[0]
    intHeight = numpy.frombuffer(buffer=strFlow,
                                 dtype=numpy.int32,
                                 count=1,
                                 offset=8)[0]

    return numpy.frombuffer(buffer=strFlow,
                            dtype=numpy.float32,
                            count=intHeight * intWidth * 2,
                            offset=12).reshape(intHeight, intWidth, 2)
...
def backwarp(tenIn, tenFlow):
    if str(tenFlow.shape) not in backwarp_tenGrid:
        tenHor = torch.linspace(
            ...
            dtype=tenFlow.dtype,
            device=tenFlow.device).view(1, 1, 1,
                                        -1).repeat(1, 1, tenFlow.shape[2], 1)
        tenVer = torch.linspace(
            ...
            dtype=tenFlow.dtype,
            device=tenFlow.device).view(1, 1, -1,
                                        1).repeat(1, 1, 1, tenFlow.shape[3])
...
class Flow(torch.nn.Module):
    ...
    def __init__(self):
        ...
        class Extractor(torch.nn.Module):
            ...
            def forward(self, tenInput):
                ...
                return [
                    tenFirst, tenSecond, tenThird, tenFourth, tenFifth,
                    tenSixth
                ]
        ...
        class Decoder(torch.nn.Module):
            ...
            def __init__(self, intChannels):
                ...
                self.netMain = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels=intChannels,
                                    out_channels=128,
                                    kernel_size=3,
                                    stride=1,
                                    padding=1),
                ...
                if objPrevious is None:
                    ...
                    tenForward = torch.nn.functional.interpolate(
                        ...
                        align_corners=False) / float(
                            objPrevious['tenForward'].shape[3]) * float(
                                intWidth)
    ...
    def forward(self, tenOne, tenTwo):
        ...
        return {
            'tenForward':
            torch.nn.functional.interpolate(input=objForward['tenForward'],
                                            size=(intHeight, intWidth),
                                            mode='bilinear',
                                            align_corners=False) *
            (float(intWidth) / float(objForward['tenForward'].shape[3])),
            'tenBackward':
            torch.nn.functional.interpolate(input=objBackward['tenForward'],
                                            size=(intHeight, intWidth),
                                            mode='bilinear',
                                            align_corners=False) *
...
class Synthesis(torch.nn.Module):
    ...
    def __init__(self):
        ...
        class Basic(torch.nn.Module):
            ...
            def __init__(self, strType, intChannels, boolSkip):
                ...
                if strType == 'relu-conv-relu-conv':
                    self.netMain = torch.nn.Sequential(
                        torch.nn.PReLU(num_parameters=intChannels[0],
                                       init=0.25),
                        torch.nn.Conv2d(in_channels=intChannels[0],
                                        out_channels=intChannels[1],
                                        kernel_size=3,
                                        stride=1,
                                        padding=1,
                                        bias=False),
                        torch.nn.PReLU(num_parameters=intChannels[1],
                                       init=0.25),
                        torch.nn.Conv2d(in_channels=intChannels[1],
        ...
        class Softmetric(torch.nn.Module):
            ...
            def __init__(self):
                ...
                self.netInput = torch.nn.Conv2d(in_channels=3,
                                                out_channels=12,
                                                kernel_size=3,
                                                stride=1,
                                                padding=1,
                                                bias=False)
                self.netError = torch.nn.Conv2d(in_channels=1,
                                                out_channels=4,
                                                kernel_size=3,
                                                stride=1,
                                                padding=1,
                ...
                for intRow, intFeatures in [(0, 16), (1, 32), (2, 64),
                                            (3, 96)]:
                    self.add_module(
                        str(intRow) + 'x0' + ' - ' + str(intRow) + 'x1',
                        Basic('relu-conv-relu-conv',
                              [intFeatures, intFeatures, intFeatures], True))
                # end
            ...
            def forward(self, tenEncone, tenEnctwo, tenFlow):
                ...
                for intRow in range(len(tenColumn) - 1, -1, -1):
                    tenColumn[intRow] = self._modules[str(intRow) + 'x' +
                                                      str(intColumn - 1) +
                                                      ' - ' + str(intRow) +
                                                      'x' + str(intColumn)](
                                                          tenColumn[intRow])
                    if intRow != len(tenColumn) - 1:
                        tenUp = self._modules[str(intRow + 1) + 'x' +
                                              str(intColumn) + ' - ' +
                                              str(intRow) + 'x' +
                                              str(intColumn)](
                                                  tenColumn[intRow + 1])
                        ...
                        if tenUp.shape[2] != tenColumn[intRow].shape[2]:
                            tenUp = torch.nn.functional.pad(input=tenUp,
                                                            pad=[0, 0, 0, -1],
                                                            mode='constant',
                                                            value=0.0)
        ...
        class Warp(torch.nn.Module):
            ...
            def __init__(self):
                ...
                self.netOne = Basic('conv-relu-conv',
                                    [3 + 3 + 32 + 32 + 1 + 1, 32, 32], True)
                self.netTwo = Basic('conv-relu-conv',
                                    [0 + 0 + 64 + 64 + 1 + 1, 64, 64], True)
                self.netThr = Basic('conv-relu-conv',
                                    [0 + 0 + 96 + 96 + 1 + 1, 96, 96], True)

            # end

            def forward(self, tenEncone, tenEnctwo, tenMetricone, tenMetrictwo,
                ...
                for intLevel in range(3):
                    if intLevel != 0:
                        ...
                        tenForward = torch.nn.functional.interpolate(
                            ...
                            mode='bilinear',
                            align_corners=False) * (
                                float(tenEncone[intLevel].shape[3]) /
                                float(tenForward.shape[3]))
                        tenBackward = torch.nn.functional.interpolate(
                            ...
                            mode='bilinear',
                            align_corners=False) * (
                                float(tenEnctwo[intLevel].shape[3]) /
                                float(tenBackward.shape[3]))
                    # end

                    tenOutput.append([
                        self.netOne, self.netTwo, self.netThr
                    ][intLevel](torch.cat([
                        softsplat.softsplat(tenIn=torch.cat(
                            [tenEncone[intLevel], tenMetricone], 1),
                                            tenFlow=tenForward,
                                            tenMetric=tenMetricone.neg().clip(
                                                -20.0, 20.0),
                                            strMode='soft'),
                        softsplat.softsplat(tenIn=torch.cat(
                            [tenEnctwo[intLevel], tenMetrictwo], 1),
                                            tenFlow=tenBackward,
                                            tenMetric=tenMetrictwo.neg().clip(
                                                -20.0, 20.0),
                                            strMode='soft')
                    ], 1)))
                # end
    ...
    def forward(self, tenOne, tenTwo, tenForward, tenBackward, fltTime):
        ...
        tenMetricone = self.netSoftmetric(tenEncone, tenEnctwo,
                                          tenForward) * 2.0 * fltTime
        tenMetrictwo = self.netSoftmetric(tenEnctwo, tenEncone,
                                          tenBackward) * 2.0 * (1.0 - fltTime)

        tenForward = tenForward * fltTime
        tenBackward = tenBackward * (1.0 - fltTime)

        tenWarp = self.netWarp(tenEncone, tenEnctwo, tenMetricone,
                               tenMetrictwo, tenForward, tenBackward)
        ...
        for intRow in range(len(tenColumn)):
            ...
            if intRow != 0:
                tenColumn[intRow] = tenColumn[intRow] + self._modules[
                    str(intRow - 1) + 'x' + str(intColumn) + ' - ' +
                    str(intRow) + 'x' + str(intColumn)](tenColumn[intRow - 1])
            # end
        # end
        ...
        for intRow in range(len(tenColumn) - 1, -1, -1):
            ...
            if intRow != len(tenColumn) - 1:
                tenUp = self._modules[str(intRow + 1) + 'x' + str(intColumn) +
                                      ' - ' + str(intRow) + 'x' +
                                      str(intColumn)](tenColumn[intRow + 1])
...
class MSFeatures(torch.nn.Module):
    ...
    def __init__(self, ):
        ...
        self.load_state_dict({
            ...
            for strKey, tenWeight in torch.load(
                'checkpoints/softsplat/softsplat.pytorch').items()
        })
    ...
    def forward(self, tenOne, tenTwo, fltTimes):
        with torch.set_grad_enabled(False):
            ...
            tenStd = (sum([
                ...
                (tenMean - tenIn.mean([1, 2, 3], True)).square()
        ...
        tenImages = [
            self.netSynthesis(tenOne, tenTwo, objFlow['tenForward'],
                              objFlow['tenBackward'], fltTime)
...
def estimate(tenOne, tenTwo, fltTimes):
    ...
    tenPreprocessedOne = torch.nn.functional.pad(input=tenPreprocessedOne,
                                                 pad=[0, intPadr, 0, intPadb],
                                                 mode='replicate')
    tenPreprocessedTwo = torch.nn.functional.pad(input=tenPreprocessedTwo,
                                                 pad=[0, intPadr, 0, intPadb],
                                                 mode='replicate')

    return [
        tenImage[0, :, :intHeight, :intWidth].cpu() for tenImage in netNetwork(
            tenPreprocessedOne, tenPreprocessedTwo, fltTimes)
    ]
