...
class WindowCrossAttention(nn.Module):
    ...
    def forward(self, x, y, mask_x=None, mask_y=None):
        ...
        if mask_x is not None:
            nW = mask_x.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask_x.unsqueeze(
                1
            ).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
            attn = self.softmax(attn)
        ...
        if mask_y is not None:
            nW = mask_y.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask_y.unsqueeze(
                1
            ).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, N, N)
            attn = self.softmax(attn)
...
class TFL(nn.Module):
    ...
    def calculate_mask2(self, x_size):
        ...
        mask_windows_down = (
            mask_windows_down.view(self.window_size * self.window_size, -1)
            .permute(1, 0)
            .contiguous()
    ...
    def forward(self, x, x_size):
        ...
        if not self.use_crossattn:
            ...
            x_windows_down = (
                x_windows_down.permute(0, 3, 2, 1)
                .contiguous()
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        x = (
            x.transpose(1, 2).contiguous().view(B, self.embed_dim, x_size[0], x_size[1])
        )  # B Ph*Pw C
        return x
...
class TFModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        dpr1 = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[1]))
        ]  # stochastic depth decay rule
        dpr2 = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[2]))
        ]  # stochastic depth decay rule
        dpr3 = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(depths[3]))
        ]  # stochastic depth decay rule
        ...
        for i_layer in range(num_layers):
            layer = RTFL(
                ...
                drop_path=dpr0[
                    sum(depths[0][:i_layer]) : sum(depths[0][: i_layer + 1])
        ...
        for i_layer in range(num_layers):
            layer = RTFL(
                ...
                attn_drop=attn_drop_rate,
                drop_path=dpr1[
                    sum(depths[1][:i_layer]) : sum(depths[1][: i_layer + 1])
                ],  # no impact on SR results
                norm_layer=norm_layer,
                downsample=None,
                use_checkpoint=use_checkpoint,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(
                ...
                attn_drop=attn_drop_rate,
                drop_path=dpr2[
                    sum(depths[2][:i_layer]) : sum(depths[2][: i_layer + 1])
                ],  # no impact on SR results
                norm_layer=norm_layer,
                downsample=None,
                use_checkpoint=use_checkpoint,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(
                ...
                attn_drop=attn_drop_rate,
                drop_path=dpr2[
                    sum(depths[3][:i_layer]) : sum(depths[3][: i_layer + 1])
                ],  # no impact on SR results
                norm_layer=norm_layer,
                downsample=None,
                use_checkpoint=use_checkpoint,
        ...
        self.conv_after_body0 = nn.Sequential(
            nn.Conv2d(embed_dim + fuse_c * 2, embed_dim, 3, 2, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
        self.conv_after_body1 = nn.Sequential(
            nn.Conv2d(embed_dim + fuse_c * 4, embed_dim, 3, 2, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
        self.conv_after_body2 = nn.Sequential(
            nn.Conv2d(embed_dim + fuse_c * 8, embed_dim, 3, 2, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
        ...
        self.conv_up0 = nn.Sequential(
            nn.ConvTranspose2d(embed_dim + fuse_c * 16, embed_dim, 4, 2, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
        self.conv_up1 = nn.Sequential(
            nn.ConvTranspose2d(2 * embed_dim, embed_dim, 4, 2, 1),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
        )
