...
class XVFInet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.device = torch.device(
            'cuda:' + str(args.gpu) if torch.cuda.is_available() else
            'cpu')  # will be used as "x.to(device)"
        self.nf = args.nf
        ...
        self.channel_converter = nn.Sequential(
            nn.Conv3d(self.in_channels, self.nf, [1, 3, 3], [1, 1, 1],
                      [0, 1, 1]), nn.ReLU())

        self.rec_ext_ds_module = [self.channel_converter]
        self.rec_ext_ds = nn.Conv3d(self.nf, self.nf, [1, 3, 3], [1, 2, 2],
                                    [0, 1, 1])
        ...
        self.rec_ext_ds_module.append(RResBlock2D_3D(args,
                                                     T_reduce_flag=False))
        self.rec_ext_ds_module = nn.Sequential(*self.rec_ext_ds_module)

        self.rec_ctx_ds = nn.Conv3d(self.nf, self.nf, [1, 3, 3], [1, 2, 2],
                                    [0, 1, 1])
    ...
    def forward(self, x, t_value, is_training=True):
        ...
        if is_training:
            ...
            out_l_list.append(out_l)
            flow_refine_l_list.append(flow_refine_l)
            return out_l_list[::-1], flow_refine_l_list[::-1], occ_0_l0, torch.mean(
                x,
...
class VFInet(nn.Module):
    ...
    def forward(self, x, feat_x, flow_l_prev, t_value, level, is_training):
        ...
        if level == 0:
            ...
            x_l = F.interpolate(x_l,
                                scale_factor=(1.0 / l, 1.0 / l),
                                mode='bicubic',
                                align_corners=False)
        '''
		Down pixel-shuffle
        ...
        if flow_l_prev is None:
            ...
            up_flow_l_prev = 2.0 * F.interpolate(flow_l_prev.detach(),
                                                 scale_factor=(2, 2),
                                                 mode='bilinear',
                                                 align_corners=False)
            warped_feat1_l = self.bwarp(feat1_l, up_flow_l_prev[:, :2, :, :])
            warped_feat0_l = self.bwarp(feat0_l, up_flow_l_prev[:, 2:, :, :])
            flow_l_tmp = self.conv_flow2(
                torch.cat([
                    self.conv_flow1(torch.cat([feat0_l, warped_feat1_l],
                                              dim=1)),
                    self.conv_flow1(torch.cat([feat1_l, warped_feat0_l],
                                              dim=1)), up_flow_l_prev
                ],
                          dim=1))
        ...
        flow_t1_l = (1 - t_value) * (
            (1 - t_value) * flow_forward) - (t_value) * (
                (1 - t_value) * flow_backward
        ...
        flow_t0_l = (1 - mask_) * flow_t0_l + mask_ * (
            flow_t0_l.clone() /
            (norm_l.clone() +
             (1 - mask_)))  # Divide the numerator with denominator in Eq.(1)
        flow_t1_l = (1 - mask_) * flow_t1_l + mask_ * (
            flow_t1_l.clone() /
            (norm_l.clone() +
             (1 - mask_)))  # Divide the numerator with denominator in Eq.(2)
        ...
        flow_refine_l = torch.cat(
            [feat0_l, warped0_l, warped1_l, feat1_l, flow_t0_l, flow_t1_l],
            dim=1)
        flow_refine_l = self.conv_flow3(flow_refine_l) + torch.cat(
            [flow_t0_l, flow_t1_l], dim=1)
        ...
        ## Flow upscale
        flow_t0_l = self.scale * F.interpolate(
            flow_t0_l,
            scale_factor=(self.scale, self.scale),
            mode='bilinear',
            align_corners=False)
        flow_t1_l = self.scale * F.interpolate(
            flow_t1_l,
            scale_factor=(self.scale, self.scale),
            mode='bilinear',
            align_corners=False)
        ...
        refine_out = self.refine_unet(
            torch.cat([
                F.pixel_shuffle(
                    torch.cat([feat0_l, feat1_l, warped0_l, warped1_l], dim=1),
                    self.scale), x_l[:, :, 0, :, :], x_l[:, :, 1, :, :],
                warped_img0_l, warped_img1_l, flow_t0_l, flow_t1_l
            ],
                      dim=1))
        ...
        out_l = (
            1 - t_value
        ) * occ_0_l * warped_img0_l + t_value * occ_1_l * warped_img1_l
        out_l = out_l / ((1 - t_value) * occ_0_l +
                         t_value * occ_1_l) + refine_out[:, 1:4, :, :]
    ...
    def sample_one(self, img, shiftx, shifty, weight):
        ...
        flat_basex = torch.arange(0, H, requires_grad=False).view(
            -1, 1)[None, None].to(self.device).long().repeat(N, C, 1,
                                                             W).view(-1)
        flat_basey = torch.arange(0, W, requires_grad=False).view(
            1, -1)[None, None].to(self.device).long().repeat(N, C, H,
                                                             1).view(-1)
        flat_weight = weight.view(-1)
        flat_img = img.contiguous().view(-1)

        # The corresponding positions in I1
        idxn = torch.arange(0, N, requires_grad=False).view(N, 1, 1, 1).to(
            self.device).long().repeat(1, C, H, W).view(-1)
        idxc = torch.arange(0, C, requires_grad=False).view(1, C, 1, 1).to(
            self.device).long().repeat(N, 1, H, W).view(-1)
        ...
        one_warp.put_(ids_mask,
                      torch.masked_select(flat_weight, mask),
                      accumulate=True)
...
class RefineUNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.enc1 = nn.Conv2d(
            (4 * self.nf) // self.scale // self.scale + 4 * args.img_ch + 4,
            self.nf, [4, 4], 2, [1, 1])
        ...
        self.dec1 = nn.Conv2d(4 * self.nf + 2 * self.nf, 2 * self.nf, [3, 3],
                              1, [1, 1])  ## input concatenated with enc2
        self.dec2 = nn.Conv2d(2 * self.nf + self.nf, self.nf, [3, 3], 1,
                              [1, 1])  ## input concatenated with enc1
        self.dec3 = nn.Conv2d(self.nf, 1 + args.img_ch, [3, 3], 1,
                              [1, 1])  ## input added with warped image
...
class RResBlock2D_3D(nn.Module):
    ...
    def __init__(self, args, T_reduce_flag=False):
        ...
        if T_reduce_flag:
            self.reduceT_conv = nn.Conv3d(self.nf, self.nf, [3, 1, 1], 1,
                                          [0, 0, 0])
